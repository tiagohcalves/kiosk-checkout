...
class TestModels:
    ...
    def test_create_item(self, db_session):
        ...
        # Then create an item
        item = Item(
            name="Test Item", price=12.99, image_id="item.jpg", category_id=category.id
        )
        db_session.add(item)
        db_session.commit()
    ...
    def test_item_category_relationship(self, db_session):
        ...
        # Create item
        item = Item(name="Test Item", price=12.99, category_id=category.id)
        db_session.add(item)
        db_session.commit()
    ...
    def test_create_order(self, db_session):
        ...
        order = Order(
            total=25.98, payment_key="abc123", payment_data='{"card": "****1234"}'
        )
    ...
    def test_cascade_delete_category_items(self, db_session):
        ...
        try:
            ...
            remaining_items = (
                db_session.query(Item).filter(Item.category_id == category_id).all()
            )
    ...
    def test_nullable_fields(self, db_session):
        ...
        item = Item(name="Minimal Item", price=9.99, category_id=category.id)
        db_session.add(item)
        db_session.commit()
