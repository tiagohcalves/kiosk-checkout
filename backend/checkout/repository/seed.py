...
def seed_database(menu_data: dict):
    ...
    try:
        ...
        print(f"Successfully seeded database with {len(menu_data['categories'])} categories and {len(menu_data['items'])} items")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Seed the database with menu data from JSON file")
    parser.add_argument(
        "json_file", 
        nargs='?',
